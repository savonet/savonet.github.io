...
Type:
```
(ffmpeg.filter.graph, source(audio=ffmpeg.audio.raw('a),
 video='b, midi='c)) -> ffmpeg.filter.audio
```

Arguments:

- `(unlabeled)` (of type `ffmpeg.filter.graph`)
- `(unlabeled)` (of type `source(audio=ffmpeg.audio.raw('a), video='b, midi='c)`)
...
Type:
```
(ffmpeg.filter.graph, source(audio='a,
 video=ffmpeg.video.raw('b), midi='c)) ->
ffmpeg.filter.video
```
...
- `(unlabeled)` (of type `ffmpeg.filter.graph`)
- `(unlabeled)` (of type `source(audio='a, video=ffmpeg.video.raw('b), midi='c)`)
...
- `headers` (of type `[string * string]`, which defaults to `[("User-Agent", "Liquidsoap/2.0.0+git@55d5a7e8 (Unix; OCaml 4.10.0)")]`): Additional headers.
- `(unlabeled)` (of type `[string * string]`)
...
- `extract_replaygain` (of type `string`, which defaults to `"/usr/local/share/liquidsoap/git/55d5a7e8/bin/extract-replaygain"`): The extraction program.
...
Type:
```
(?id : string?, ?buffer : float, ffmpeg.filter.graph,
 ffmpeg.filter.audio) ->
source(audio=ffmpeg.audio.raw('a), video=none, midi=none)
```
...
- `user_agent` (of type `string`, which defaults to `"Liquidsoap/2.0.0+git@55d5a7e8 (Unix; OCaml 4.10.0)"`): User agent.
- `(unlabeled)` (of type `{string}`): URL to decode.
...
Type:
```
(?id : 'a?, ?max_buffer : float, ?device : string) ->
source(audio=none, video='c, midi='d)
```

Arguments:

- `id` (of type `'a?`, which defaults to `null`): Force the value of the source ID.
...
Type:
```
(?id : string?, ?prefetch : int, ?polling_delay : float,
 ?timeout : float, (() -> request?)) -> source(audio='a,
video='b, midi='c)
```
...
- `id` (of type `string?`, which defaults to `null`): Force the value of the source ID.
- `prefetch` (of type `int`, which defaults to `1`): Number of tracks to prepare in advance.
- `polling_delay` (of type `float`, which defaults to `1.`): How often the queue should be checkelid and filled in seconds.
- `timeout` (of type `float`, which defaults to `20.`): Timeout for resolving requests in seconds.
- `(unlabeled)` (of type `() -> request?`): Function returning the next request.
...
- `polling_delay` (of type `float`, which defaults to `1.`): How often the queue should be checkelid and filled in seconds.
- `timeout` (of type `float`, which defaults to `20.`): Timeout for resolving requests in seconds.
- `(unlabeled)` (of type `() -> [request?]`): Function returning the list of next requests.
...
- `id` (of type `string?`, which defaults to `null`): Force the value of the source ID.
- `duration` (of type `float`, which defaults to `-1.`): Duration in seconds (negative means infinite).
...
(?id : string?, ?check_next : ((request) -> bool)?,
 ...
 ?length : float, ?loop : bool, ?mime_type : string?,
...
- `default_duration` (of type `float`, which defaults to `30.`): When unknown, assume this duration (in sec.) for files.
- `length` (of type `float`, which defaults to `10.`): How much audio (in sec.) should be queued in advance.
...
(?id : string?, ?check_next : ((request) -> bool)?,
 ?conservative : bool, ?default_duration : float,
 ?length : float, ?loop : bool, ?mime_type : 'a?,
 ?mode : string, ?native : bool, ?on_done : (() -> unit),
 ?timeout : float, [string]) -> source(audio='b,
video='c, midi='d)
```
...
- `length` (of type `float`, which defaults to `10.`): How much audio (in sec.) should be queued in advance.
- `loop` (of type `bool`, which defaults to `true`): Loop on the playlist.
- `mime_type` (of type `'a?`, which defaults to `null`): Default MIME type for the playlist. Empty string means automatic detection.
...
Type:
```
(?id : string?, ?available : (() -> bool),
 ?retry_delay : float, ?conservative : bool,
 ?default_duration : float, ?length : float,
 ?timeout : float, ?native : bool, (() -> request?)) ->
source(audio='a, video='b, midi='c)
```
...
- `timeout` (of type `float`, which defaults to `20.`): Timeout (in sec.) for a single download.
- `native` (of type `bool`, which defaults to `false`): Use native implementation.
- `(unlabeled)` (of type `() -> request?`): Function producing requests.
...
- `on_shutdown` (of type `((() -> unit)) -> unit`): Register a function to be called when source shuts down.
- `on_track` (of type `((([string * string]) -> unit)) -> unit`): Call a given handler on new tracks.
- `prefetch` (of type `() -> unit`): Try feeding the queue with a new request (this method can take long to return and should usually be run in a separate thread).
- `queue_length` (of type `() -> float`): Size of the queue in seconds.
- `queue_size` (of type `() -> int`): Number of requests in the queue.
- `remaining` (of type `() -> float`): Estimation of remaining time in the current track.
- `seek` (of type `(float) -> float`): Seek forward, in seconds (returns the amount of time effectively seeked).
- `self_sync` (of type `() -> bool`): Is the source currently controling its own real-time loop.
- `shutdown` (of type `() -> unit`): Deactivate a source.
...
(?id : string?, ?available : {bool},
 ?conservative : bool, ?default_duration : float,
 ?length : float, ?retry_delay : {float},
 ?timeout : float, (() -> [request])) -> source(audio='a,
video='b, midi='c)
```
...
- `conservative` (of type `bool`, which defaults to `false`): If true, estimated remaining time on the current track is not considered when computing queue length.
- `default_duration` (of type `float`, which defaults to `30.`): When unknown, assume this duration (in sec.) for files.
- `length` (of type `float`, which defaults to `40.`): How much audio (in sec.) should be queued in advance.
- `retry_delay` (of type `{float}`, which defaults to `0.1`): Retry after a given time (in seconds) when callback returns an empty list.
- `timeout` (of type `float`, which defaults to `20.`): Timeout (in sec.) for a single download.
- `(unlabeled)` (of type `() -> [request]`)
...
- `on_track` (of type `((([string * string]) -> unit)) -> unit`): Call a given handler on new tracks.
- `remaining` (of type `() -> float`): Estimation of remaining time in the current track.
- `resolve` (of type `() -> bool`): Resolve the request (this is useful to make sure that the source will be available in advance). This function returns `true` if we were able to successfully perform resolution.
- `seek` (of type `(float) -> float`): Seek forward, in seconds (returns the amount of time effectively seeked).
- `self_sync` (of type `() -> bool`): Is the source currently controling its own real-time loop.
- `shutdown` (of type `() -> unit`): Deactivate a source.
- `skip` (of type `() -> unit`): Skip to the next track.
...
(?id : string?, ?conservative : bool,
 ?default_duration : float, ?fallible : bool,
 ?length : float, ?timeout : float, string) ->
source(audio='a, video='b, midi='c)
```
...
(?id : string?, ?fallible : bool,
 ?on_start : (() -> unit), ?on_stop : (() -> unit),
 ?restart_on_error : bool, ?start : bool, url : string,
...
Type:
```
(?id : 'a?, low : {float}, high : {float}, ?q : float,
...
- `(unlabeled)` (of type `source(audio='b, video='c, midi='d)`): Force the value of the source ID.
...
Type:
```
(?id : 'a?, ?target : {float}, ?up : {float},
 ?down : {float}, ?gain_min : float, ?gain_max : float,
 ?lookahead : {float}, ?window : {float},
 ?threshold : {float}, ?track_sensitive : bool,
 ?debug : float??, source(audio=pcm('b), video='c,
 midi='d)) -> source(audio=pcm('b), video='c, midi='d)
```
...
- `(unlabeled)` (of type `source(audio=pcm('b), video='c, midi='d)`): Source to normalize.
...
- `replay_delay` (of type `float?`, which defaults to `null`): Replay track marks and metadata from the input source on the output after a given delay. If negative (default) close and flush the process on each track and metadata to get an exact timing.
- `restart` (of type `bool`, which defaults to `true`): Restart process when exited.
...
Type:
```
(?id : 'a?, ?threshold : float, ?max_blank : float,
 ?min_noise : float, ?track_sensitive : bool,
 source(audio='b, video='c, midi='d).{skip : (() -> unit)
...
- `min_noise` (of type `float`, which defaults to `0.`): Minimum duration of noise required to end silence, in seconds.
- `track_sensitive` (of type `bool`, which defaults to `true`): Reset blank counter at each track.
- `(unlabeled)` (of type `source(audio='b, video='c, midi='d).{skip : (() -> unit)}`)
...
Type:
```
(?id : string?, ?initial : bool, float, source(audio='a,
 video='b, midi='c)) -> source(audio='a, video='b,
midi='c)
```
...
- `mime` (of type `string`): MIME type for the cover.
...
(?id : string?, ?conservative : bool,
 ...
 ?length : float, ?native : bool, ?queue : [request],
 ?timeout : float) -> source(audio='a, video='b, midi='c)
```
...
- `resurection_time` (of type `float`, which defaults to `1.`): When track sensitive and the source is unavailable, how long we should wait before trying to update source again (negative means never).
...
- `id` (of type `string?`, which defaults to `null`): Force the value of the source ID.
- `width` (of type `{int}`, which defaults to `-1`): Scale to width (negative means original width).
- `height` (of type `{int}`, which defaults to `-1`): Scale to height (negative means original height).
...
(?color : int, ?font : string, ?size : int, ?x : int,
 ?y : int, string?, source(audio='a,
 video=ffmpeg.video.raw('b), midi=none)) ->
source(audio='a, video=ffmpeg.video.raw('d), midi=none)
```
...
- `(unlabeled)` (of type `source(audio='a, video=ffmpeg.video.raw('b), midi=none)`): Source on which the text should be added.
...
- `id` (of type `string?`, which defaults to `null`): Force the value of the source ID.
- `height` (of type `{int}`, which defaults to `-1`): Target height (negative means original height).
- `width` (of type `{int}`, which defaults to `-1`): Target width (negative means original width).
- `x` (of type `{int}`, which defaults to `0`): x offset.
- `y` (of type `{int}`, which defaults to `0`): y offset.
- `(unlabeled)` (of type `source(audio='a, video='b, midi='c)`)
...
Type:
```
(?id : 'a?, ?pattern : string, ?max_buffer : float,
 ?duration : 'b?) -> source(audio=none, video='d,
midi='e)
```
...
- `max_buffer` (of type `float`, which defaults to `0.5`): Maximum data buffer in seconds
- `duration` (of type `'b?`, which defaults to `null`): Duration of the source.
...
### `string.escape_annotation`

Escape a string so that it is suitable for use as value for the `annotate:` protocol.
...
Retrieve a character in a string.
...
Unescape strings. This function is the inverse of `string.escape`.
...
Escape special characters in an UTF8 string.
...
### `string.utf8.unescape`

Unescape UTF8 strings. This function is the inverse of `string.utf8.escape`.
...
Read the tags from an MP3 file using the builtin functions. Only ID3v2 tags are supported for now.
...
- `(unlabeled)` (of type `string`): MP3 file of which the metadata should be read.

### `file.mp3.parse_apic`

Parse APIC ID3v2 tags (such as those obtained in the APIC tag from `file.mp3.tags`). The returned values are: mime, picture type, description, and picture data.
